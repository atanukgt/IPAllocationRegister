 ...
 * @author Souvik
 */
@WebServlet(name = "shiftIP", urlPatterns = {"/shiftIP"})
public class shiftIP extends HttpServlet {
...
while(rs.next()){
            ...
            {
        reply = JOptionPane.showConfirmDialog(null, "Do you Want to Shift This IP to Expired details Table ?", "Select", JOptionPane.YES_NO_OPTION );
...
if(i>0 && i2>0)                
{
    JOptionPane.showMessageDialog(null, "IP Address has been Shifted to Expired Allocations.");
    response.sendRedirect("Menupage.jsp");
}              
else                
{
    JOptionPane.showMessageDialog(null, "Insert Unsuccessfull.. Plz Try Again");
    response.sendRedirect("addcheck.jsp");
}
                stmt2.close();
                con2.close();
...
{          out.println(e);               
        ...
        if (reply == JOptionPane.NO_OPTION) 
        {
           ...
           response.sendRedirect("newformat.jsp");
           //return;
           //remove(reply);
        }
...
{          out.println(e);               
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet shiftIP</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet shiftIP at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
